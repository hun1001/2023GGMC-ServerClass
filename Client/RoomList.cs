...
public partial class RoomList : Form
{
    ...
    // 방입장하기
    private async void btn_entrance_Click(object sender, EventArgs e)
    {
        if (listBox_roomList.SelectedItem == null)
            return;

        EnterRoomRequestPacket packet = new EnterRoomRequestPacket(listBox_roomList.SelectedItem.ToString());
        await Singleton.Instance.Socket.SendAsync(packet.Serialize(), System.Net.Sockets.SocketFlags.None);
    }
